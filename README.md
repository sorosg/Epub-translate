...
Az EPUB Fordító Rendszer egy **öntanuló EPUB-fordító** angolról magyarra. Két AI-motor közül választhatsz: **DeepSeek Pro (távoli, gyors, jó minőség)** vagy **helyi Ollama (GPU-val, adatvédelem/offline)**. A rendszer közös könyvtárat, glosszáriumot és fordítási memóriát épít a folyamatosan javuló minőségért.
...
- ✅ **Két AI-motor** – DeepSeek Pro (távoli, alacsony költség) vagy helyi Ollama (GPU-val)
- ✅ **Asztali (desktop) kiadás** – Windows/Mac, Docker nélkül (fejlesztés alatt, scaffold kész)
- ✅ **Checkpoint/folytatás** – a fordítás megszakadás után onnan folytatódik, ahol abbahagyta
- ✅ **100% Ingyenes (helyi AI)** – A DeepSeek R1/Ollama modellek a saját gépeden futnak
...
## 🖥️ Asztali (Desktop) alkalmazás — fejlesztés alatt

A rendszer **önálló asztali alkalmazásként** is futhat Windows/macOS-en, Docker és helyi AI nélkül.

- **Stack**: Electron (ablak) + PyInstaller (Flask backend sidecar) + SQLite (egyfelhasználós).
- **AI**: alapból **DeepSeek Pro (távoli)**; a helyi (GPU) fordítás opcionális, és a modellt a VRAM-hoz szabja a rendszer.
- **Állapot**: a scaffold (`desktop/`) és a platformfüggetlen backend (SQLite, `DESKTOP_MODE`, GPU-detektálás) elkészült; a telepítő-build (`.exe`/`.dmg`) a CI-vel futtatható.
- Részletek: `desktop/README.md`.

---
## 📊 Verzió Történet

### v2.6.x (2026-08-15/16)
Könyvtár-jóváhagyás, OpenLibrary metaadat-pótlás, több fájl feltöltés,
sorozat-kinyerés, régi könyvek szerkesztése, Budapest időzóna, fordítási log
javítás, Desktop alapozó (platformfüggetlen backend + Electron scaffold + GPU-detektálás).

### v2.5.x (2026-08-14/15)
Checkpoint/folytatás, NER majd annak kikapcsolása, tegezés/magázás,
email csatolmány, review kikapcsolás, TM-fixek.
